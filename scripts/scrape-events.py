...
SOURCES=json.loads((ROOT/'event-sources.json').read_text()); OUT=ROOT/'events.json'; TZ=ZoneInfo('Europe/London'); NOW=datetime.now(TZ)
...
def valid_title(title):
    ...
    return True
def slug_title(url):
    slug=urlparse(url).path.rstrip('/').split('/')[-1]; slug=re.sub(r'^[A-Za-z0-9_-]+-', '', slug); slug=re.sub(r'[-_]+',' ',slug).strip()
    return clean(slug.title()) if slug else ''
...
def dom_events(html,venue,page_url,detail_only=False):
    ...
    for node in soup.find_all(['article','li','div'],limit=15000):
        ...
        h=node.find(['h1','h2','h3','h4']); title=clean(h.get_text(' ',strip=True)) if h else ''; href=page_url
        if not valid_title(title):
            title=''
            for a in links:
                t=clean(a.get_text(' ',strip=True)); u=urljoin(page_url,a['href'])
                if valid_title(t) and '/whats-on/' in urlparse(u).path:title=t;href=u;break
            if not title and links:
                for a in links:
                    u=urljoin(page_url,a['href']); candidate=slug_title(u)
                    if valid_title(candidate) and '/whats-on/' in urlparse(u).path:title=candidate;href=u;break
...
async def get_html(browser,url):
    ...
    try:
        ...
        await page.wait_for_timeout(1200);return await page.content(),page.url,None
...
async def scrape_source(browser,source):
    ...
    if err:return [],err
    venue=source['venue'];is_dome_search='brightondome.org/search/' in source['url'];events=jsonld_events(html,venue,final);events.extend(dom_events(html,venue,final,detail_only=is_dome_search))
    if venue=='Brighton Dome':
        ...
        detail,failed=await scrape_detail_urls(browser,venue,sorted(urls));events.extend(detail);print(f'Brighton Dome detail crawl: {len(urls)} pages, {len(detail)} events, {failed} failures')
    return events,None
async def main():
    ...
    scraped={}
    def add_event(store,e):
        """Deduplicate same venue/date, including short-title vs specific-title variants."""
        venue=e.get('venue','').lower(); date=e.get('date',''); tk=title_key(e.get('title',''))
        exact=(venue,date,tk)
        if exact in store:
            old=store[exact]
            if len(e.get('title',''))>len(old.get('title','')):store[exact]=e
            elif not old.get('time') and e.get('time'):store[exact]=e
            return
        # If one title is the other title plus a subtitle, keep the more specific/longer one.
        for key,old in list(store.items()):
            if key[0]!=venue or key[1]!=date:continue
            oldk=title_key(old.get('title',''))
            if oldk==tk:continue
            if oldk in tk or tk in oldk:
                chosen=e if len(e.get('title',''))>len(old.get('title','')) else old
                del store[key]; store[(venue,date,title_key(chosen.get('title','')))]=chosen
                return
        store[exact]=e
    for e in all_events:add_event(scraped,e)
    ...
    for e in existing.get('events',[]):
        ...
        if RANGE_START<=d<=RANGE_END:
            ...
            add_event(retained,e)
    for e in scraped.values():add_event(retained,e)
