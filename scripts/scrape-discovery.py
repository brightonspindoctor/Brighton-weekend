...
def extract_cards(html,page_url):
 ...
 for node in soup.find_all(['article','li','div'],limit=20000):
  ...
  for a in node.find_all('a',href=True):
   ...
   if 4<=len(t)<=180 and t.lower() not in {'more info','more info & tickets','buy tickets','book tickets','find out more','event details'}:title=t;url=urljoin(page_url,a['href']);break
  if not title:
...
async def main():
 async with async_playwright() as p:
  ...
  await browser.close()
 data=json.loads(OUT.read_text());merged={(e['venue'].lower(),e['date'],re.sub(r'[^a-z0-9]+',' ',e['title'].lower()).strip()):e for e in data.get('events',[])}
 for e in all_events:
  key=(e['venue'].lower(),e['date'],re.sub(r'[^a-z0-9]+',' ',e['title'].lower()).strip());old=merged.get(key)
  if old is None or (not old.get('time') and e.get('time')):merged[key]=e
 data['events']=sorted(merged.values(),key=lambda x:(x['date'],x.get('time') or '99:99',x['venue'],x['title']));data['venues']=sorted({e['venue'] for e in data['events']});data['updated']=NOW.date().isoformat();OUT.write_text(json.dumps(data,ensure_ascii=False,indent=2)+'\n');print(f'Merged {len(all_events)} discovery events; events.json now has {len(data["events"])} events')
