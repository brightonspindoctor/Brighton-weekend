...
begin
  ...
  if p_status is null then
    ...
    -- Avoid ON CONFLICT (event_id, user_id) here: because event_id/user_id
    -- are also OUT parameters, PostgreSQL can resolve those names as PL/pgSQL
    -- variables and raise 42702 ambiguous_column. Update first, then insert.
    update public.event_interest AS ei
       set user_name = btrim(coalesce(p_user_name,'')),
           status = p_status
     where ei.event_id = p_event_id
       and ei.user_id = p_user_id;
